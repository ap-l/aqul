...
var inc = 1 / (path_get_length(path) / 8);
var pos = 0;
// Loop through every position along the path 1 pixel at a time
while (pos < 1) {
  var px = path_get_x(path, pos);
  var py = path_get_y(path, pos);
  px = floor(px / 8)  * 8;  // Round px to the nearest grid_width (for example if grid_width = 32, and px = 69 it px would become 64 because thats the nearest multiple of 32)
  py = floor(py / 8) * 8; // Round py to the nearest grid_height
